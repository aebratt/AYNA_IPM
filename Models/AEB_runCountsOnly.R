...
#### MCMC SETTINGS ####
nb <- 1#00 #burn-in
ni <- nb+10#100000 + nb #total iterations
nt <- 1  #thin
nc <- 3  #chains
adaptInterval = 100
...
# p.link 6, 7, 8, 9, t = 9
# AEB - fixed in prep data - but check with Steffen that this is correct
...
for(link in 3:5){
  conf$addSampler(target = paste0("p.link[",link,", 1:",nyear.in,"]"), type="AF_slice")
}#s
for(link in 6:10){
  conf$addSampler(target = paste0("p.link[",link,", 1:",nyear.out,"]"), type="AF_slice")
}#s
...
conf$addSampler(target = paste0("nF[1:",nyear,"]"), type="AF_slice")
conf$printSamplers("nF")

## block all the pair count variables  
conf$removeSamplers('nP')
conf$addSampler(target = paste0("nP[1:",nyear,"]"), type="AF_slice")
conf$printSamplers("nP")
