...
YNAL.IPM <- nimbleCode({ 
  ...
  # TODO
  # rewrite this as a nimble function
  for(t in 1:(nyear-1)){
    
    # TODO - is this right??
    # juveniles are not observed
    ...
    # dead must be unobserved
    pi[6,t,1]<-0;       pi[6,t,2]<-0;              pi[6,t,3]<-0;              pi[6,t,4]<-0;             pi[6,t,5]<-0;                  pi[6,t,6]<-1;
    
    # TODO
    # what is this????
    # oh i see - this is super confusing that p and pi have different dimensions
    p[1,2,t] <- 0;
    p[1,3,t] <- 0;
    p[2,3,t] <- 0;
    p[4,2,t] <- 0;
